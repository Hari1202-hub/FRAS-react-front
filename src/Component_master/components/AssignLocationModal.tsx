...
export default function AssignLocationModal({
  ...
  const handleRemoveLocationFromServer = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Project location has been removed successfully.",
        variant: "default",
      });
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to remove project location.",
        variant: "destructive",
      });
    }
  };
  ...
  const initializeMap = (
    cur_lat = 13.090280396661807,
    cur_lng = 80.27594586748616,
  ) => {
    ...
    if (
      project &&
      project.project_lat_lng &&
      project.project_lat_lng.length > 0
    ) {
