...
const Reports = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const uniqueProjects = Array.from(
          new Map(
            reports
              .filter((r) => r.project && r.project.id)
  ...
  const getFilteredData = () => {
    ...
    if (attendaceTypeFilter) {
      filtered = filtered.filter((record) => {
        return (
          attendaceTypeFilter == "all" ||
          record.attendance_type == attendaceTypeFilter
        );
      });
    }
    if (entityFilter) {
    ...
    if (classificationFilter) {
      filtered = filtered.filter((record) => {
        return (
          ...
          record.classification.code === classificationFilter
        );
      });
    }
    if (categoryFilter) {
      filtered = filtered.filter((record) => {
        return (
          categoryFilter === "all" || record.category.code === categoryFilter
        );
      });
    }

    if (projectFilter && projectFilter !== "all") {
      // console.log("Selected Project Filter:", projectFilter);
      ...
      filtered = filtered.filter((record) => {
        ...
        return String(record.project.id) === String(projectFilter);
      });
    }
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((record) => {
        ...
        //alert(searchTerm);
        const searchMatch = record.user_login.emp_id
          .toLowerCase()
    ...
    if (searchName) {
      filtered = filtered.filter((record) => {
        const searchMatch = record.user.name
          .toLowerCase()
  ...
  const totalWorkedHours = filteredData.reduce((sum, record) => {
    ...
    if (
      typeof record.worked_hours === "string" &&
      record.worked_hours.includes(":")
    ) {
              ...
              <span className="font-semibold">
                {
                  [
                    ...new Set(
                      filteredData.map((record) => record.user_login.emp_id),
                    ),
                  ].length
                }
              </span>
            ...
            <TableBody>
              {filteredData.map((record) => (
                <TableRow key={record.id}>
                  <TableCell>{record.project?.projectid || ""}</TableCell>
                  <TableCell>
                    {record.project.projectname
                      ? ` ${record.project.projectname}`
                      : ""}
                  ...
                  <TableCell>
                    {record.checkout ? `${record.date} ${record.checkout}` : ""}
                  </TableCell>
